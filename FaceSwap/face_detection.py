...
def select_face(im, r=10, choose=True):
    ...
    if len(faces) == 1 or not choose:
        ...
        # this holds the face that comes from detector. This is the box object in the Face class.
        bbox = bbox[0]
    ...
    ret_face = points - np.asarray([[x, y]]), (x, y, w, h), im[y:y + h, x:x + w]
    print(ret_face)
    return ret_face



def give_face(im, face:Face, r=10,):#, bbox:dlib.rectangle, landmarkPoints=None):
    landmarkPoints = face.landmarks
    bbox = face.box
    # im = Image.image

    im_w, im_h = im.shape[:2]
    left, top = np.min(landmarkPoints, 0)
    right, bottom = np.max(landmarkPoints, 0)

    x, y = max(0, left - r), max(0, top - r)
    w, h = min(right + r, im_h) - x, min(bottom + r, im_w) - y

    ret_face = landmarkPoints - np.asarray([[x, y]]), (x, y, w, h), im[y:y + h, x:x + w]
    return ret_face
